...
export default {
    ...
    data() {
        return {
            ...
            phoneNumber: "",
            order: "",
            token: "5276106701:5276106701:AAGnxbgR_nScAUPmF9Yk1JPuOhtvIhjOsg0",
            chatId: "-676598452",
            fullMessage:''
        };
    },
    methods: {
        submit() {
            ...
            this.fullMessage = `Name: ${this.name}` + '\n' + `E-Mail: ${this.emailAddress}`+ '\n' +`Phone Number: ${this.phoneNumber}` + '\n' +`Order:${this.order}`;
            console.log(this.fullMessage);
            this.$http.post(`https://api.telegram.org/bot${this.token}/sendMessage?chat_id=${this.chatId}&text=${this.fullMessage}`)
                .then(response => {
                console.log("Success", response);
            }, error => {
                console.log(error);
            });
            this.$emit('submitMsg', this.fullMessage)
        }
    },
    components: {   }
}
